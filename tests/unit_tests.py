...
def expected_output(res, catch24=False, short_names=False):
    ...
    if short_names:
        ...
        assert all(col_name in ['feature', 'value', 'short_name'] for col_name in res.columns), f"{which_set} returned unexpected columns."

        # check the short names list
        assert all(isinstance(sname, str) for sname in res['short_name']), f"{which_set} expected all returned short names to be strings."
        length_of_names = len(res['short_name'])
        assert length_of_names == num_features, f"Expected {num_features} short names for {which_set}, got {length_of_names} instead."

    else:
        assert len(res.columns) == 2, f"{which_set} returned a DataFrame with an unexpected number of columns."
        # check if the keys are 'names' and 'values'
        assert all(col_name in ['feature', 'value'] for col_name in res.columns), f"{which_set} returned unexpected columns."

    # check the 'feature' column
    assert all(isinstance(fname, str) for fname in res['feature']), f"{which_set} expected feature names as str, got unexpected type."
    length_of_names = len(res['feature'])
    assert length_of_names == num_features, f"Expected {num_features} features for {which_set}, got {length_of_names} instead."
